...
sk_sp<SkImage> KawaseBlurFilter::generate(GrRecordingContext* context, const uint32_t blurRadius,
    ...
    SkImageInfo finalInfo = input->imageInfo().makeWH(scaledWidth, scaledHeight);
    sk_sp<SkSurface> surface = SkSurface::MakeRenderTarget(context, SkBudgeted::kNo, finalInfo);

    const float sigmaScale = blurRadius * kInputScale * 0.5f; // Adjust the scale factor as needed
    ...
    const float glassBlurScale = 0.5f; // Define the scale factor for the glass blur
